...
export type EmailType =
  | "booking_confirmation" | "approved" | "cancelled" | "reminder" | "payment"
  | "invoice" | "contact" | "admin_alert" | "venue" | "restaurant" | "newsletter"
  | "password_reset" | "otp" | "welcome" | "review_request" | "offer" | "generic";

export type RenderedEmail = { subject: string; html: string };

/** Render a luxury HTML email + subject for a given type and free-form data. */
export function renderEmail(type: string, d: Record<string, any> = {}): RenderedEmail {
  ...
  switch (type) {
    case "booking_confirmation":
      return {
        ...
        html: shell(
          ...
          rowsFrom([
            ["Booking ID", d.bookingId], ["Guest", name], ["Room", d.roomType || d.room],
            ["Check-in", d.checkIn], ["Check-out", d.checkOut], ["Guests", d.guests],
            ["Special Requests", d.requests], ["Payment Status", d.paymentStatus || "Pending"],
    ...
    case "approved":
      return {
        ...
        html: shell("Your booking has been approved",
          `Dear ${name}, great news — your reservation has been approved and confirmed.`,
          rowsFrom([["Booking ID", d.bookingId], ["Room", d.roomType || d.room], ["Check-in", d.checkIn], ["Check-out", d.checkOut], ["Guests", d.guests]])),
      };
    case "cancelled":
      return {
        subject: `Booking Cancelled — ${BRAND}`,
        html: shell("Your booking has been cancelled",
          `Dear ${name}, your reservation has been cancelled. If this was not requested by you, please contact us at ${PHONE}.`,
          rowsFrom([["Booking ID", d.bookingId], ["Room", d.roomType || d.room], ["Check-in", d.checkIn], ["Check-out", d.checkOut], ["Reason", d.reason]])),
      };
    case "reminder":
      return {
        subject: `Your stay is coming up — ${BRAND}`,
        html: shell("We look forward to welcoming you",
    ...
    case "payment":
      return {
        ...
        html: shell("Payment received",
          ...
          rowsFrom([["Amount Paid", d.amount], ["Reference", d.reference], ["Booking ID", d.bookingId], ["Details", d.description]])),
      };
    case "invoice":
      return {
        subject: `Invoice — ${BRAND}`,
        html: shell("Your invoice",
          `Dear ${name}, please find your invoice details below.`,
          rowsFrom([["Invoice No.", d.invoiceNo || d.reference], ["Booking ID", d.bookingId], ["Amount", d.amount], ["Tax", d.tax], ["Total", d.total || d.amount], ["Status", d.paymentStatus || "Paid"]])),
      };
    case "contact":
      return {
        subject: `We received your message — ${BRAND}`,
        html: shell("We received your message",
    ...
    case "admin_alert":
      return {
        ...
        html: shell("New booking received",
          "A new booking has been submitted on the website.",
          rowsFrom([["Guest", name], ["Phone", d.phone], ["Email", d.email], ["Room", d.roomType || d.room], ["Check-in", d.checkIn], ["Check-out", d.checkOut], ["Guests", d.guests], ["Notes", d.requests || d.notes]]) +
          `<tr><td colspan="2">${button("Open Admin Dashboard", ADMIN_URL)}</td></tr>`),
      };
    case "venue":
      return {
        subject: `Venue Booking Request — ${BRAND}`,
        html: shell("Venue booking request received",
          `Dear ${name}, thank you for your interest in hosting your event at ${BRAND}. Our events team will contact you shortly.`,
          rowsFrom([["Venue", d.venue], ["Event Type", d.eventType], ["Event Date", d.eventDate], ["Expected Guests", d.guests], ["Phone", d.phone], ["Special Request", d.requests]])),
      };
    case "restaurant":
      return {
        subject: `Restaurant Reservation — ${BRAND}`,
        html: shell("Your table reservation",
          `Dear ${name}, thank you for choosing to dine with us at ${BRAND}.`,
          rowsFrom([["Date", d.date || d.eventDate], ["Time", d.time], ["Party Size", d.guests], ["Phone", d.phone], ["Special Request", d.requests]])),
      };
    case "newsletter":
      return {
        subject: d.subject || `${BRAND} — Newsletter`,
        html: shell(d.title || "From all of us at " + BRAND,
          d.intro || `Dear ${name}, here is the latest news from ${BRAND}.`,
          (d.body ? `<tr><td colspan="2" style="padding:8px 0;color:${CHARCOAL};font-size:14px;line-height:1.7">${String(d.body).replace(/\n/g, "<br/>")}</td></tr>` : "") +
          (d.ctaUrl ? `<tr><td colspan="2">${button(d.ctaLabel || "Discover More", d.ctaUrl)}</td></tr>` : "")),
      };
    case "password_reset":
      return {
    ...
    case "review_request":
      return {
        ...
        html: shell("We'd love your feedback",
    ...
    case "offer":
      return {
        ...
        html: shell(d.title || "An exclusive offer for you",
          `Dear ${name}, ${d.intro || "we are delighted to share an exclusive offer with you."}`,
          rowsFrom([["Offer", d.offer || d.title], ["Details", d.body || d.description], ["Valid Till", d.validTill]]) +
