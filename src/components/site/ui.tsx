...
export function LuxeButton({
  ...
  const base =
    "inline-flex items-center justify-center rounded-full px-7 py-3.5 text-xs font-medium uppercase tracking-[0.2em] transition-all duration-300";
  const styles = {
  ...
  return (
    <button type={type ?? "button"} onClick={onClick} className={`${base} ${styles}`}>
      {children}
    </button>
  );
}
...
export function SectionHeading({
  ...
  return (
    <Reveal className={center ? "mx-auto max-w-2xl text-center" : "max-w-2xl"}>
      ...
      {sub && (
        <p className={`mt-5 text-base ${light ? "text-ivory/70" : "text-muted-foreground"}`}>
          {sub}
        </p>
      )}
    </Reveal>
          ...
          <div className="mt-9 flex flex-wrap justify-center gap-4">
            ...
            <Link to="/contact">
              <LuxeButton variant="outline">
                <span className="text-ivory">Contact Us</span>
              </LuxeButton>
            </Link>
          </div>
        </Reveal>
