...
const NAV: { section: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [{ to: "/admin/emails", label: "Email Center", icon: Mail }],
  },
];
    ...
    <div className="min-h-screen bg-[#FAFAF8] text-[#1a1a1a]">
      ...
      >
        <div className="flex h-16 items-center gap-2.5 border-b border-white/5 px-5">
          ...
          <button className="ml-auto lg:hidden" onClick={() => setOpenMobile(false)}>
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          {NAV.map((group) => {
            ...
            return (
              <div key={group.section} className="mb-5">
                <p className="mb-2 px-3 text-[10px] uppercase tracking-[0.18em] text-white/30">
                  {group.section}
                </p>
                {items.map((item) => {
