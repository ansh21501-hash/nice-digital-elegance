...
function emptyForm(fields: Field[]) {
  ...
  for (const f of fields)
    o[f.name] = f.default ?? (f.type === "boolean" ? false : f.type === "tags" ? [] : "");
  return o;
}
...
export function ResourceManager({
  ...
  const { data = [], isLoading } = useRows<Record<string, unknown>>(table, { orderBy, ascending });
  ...
  const save = async () => {
    ...
    try {
      ...
      await upsertRow(table, payload, editing?.id as string | undefined);
    ...
    <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-black/5 bg-[#FAFAF8] text-left text-xs uppercase tracking-wider text-muted-foreground">
                {columns.map((c) => (
                  <th key={c.name} className="whitespace-nowrap px-4 py-3 font-medium">
                    {c.label}
                  </th>
                ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit" : "New"} record</DialogTitle>
          </DialogHeader>
                  ...
                  >
                    ...
                    <SelectContent>
                      {(f.options ?? []).map((o) => {
                        ...
                        return (
                          <SelectItem key={val || "_"} value={val || "_none"}>
                            {lbl || "—"}
                          </SelectItem>
                        );
                      })}
