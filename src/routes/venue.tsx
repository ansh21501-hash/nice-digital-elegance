  ...
  head: () => ({
    meta: [
      { title: "Party & Event Halls — Nice Hotel And Restaurant, Mansa" },
      {
        name: "description",
        content:
          "Elegant party hall and meeting room for weddings, kitty parties and corporate events in Mansa. Capacity up to 100 guests with catering and decor.",
      },
          ...
          <Reveal key={v.slug} delay={i * 0.1}>
            <div className="group flex h-full flex-col overflow-hidden rounded-2xl bg-card shadow-card transition hover:shadow-luxe">
              <div className="relative h-56 overflow-hidden">
                ...
                <span className="absolute right-4 top-4 flex items-center gap-1 rounded-full bg-ivory/90 px-3 py-1 text-xs text-charcoal">
              ...
              <div className="flex flex-1 flex-col p-7">
                ...
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {v.amenities.slice(0, 4).map((a) => (
                    <span key={a} className="rounded-full bg-beige px-3 py-1 text-xs text-brown">
                      {a}
                    </span>
                  ))}
                </div>
