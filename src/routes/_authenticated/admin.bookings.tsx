...
const fields: Field[] = [
  ...
  { name: "amount", label: "Amount (₹)", type: "number" },
  {
    ...
    label: "Status",
    type: "select",
    options: ["pending", "confirmed", "checked_in", "checked_out", "completed", "cancelled"],
    default: "pending",
  },
  {
    name: "payment_status",
  ...
  },
  {
    ...
    label: "Source",
    type: "select",
    options: ["website", "walk-in", "phone", "online"],
    default: "website",
  },
...
const columns: Column[] = [
  {
    ...
    render: (r) => (
      <div>
        ...
        <p className="text-xs text-muted-foreground">{r.guest_phone || r.guest_email || "—"}</p>
      </div>
    ),
  },
  {
    name: "room_type",
    label: "Rooms",
    render: (r) => <BookingRoomsCell bookingId={r.id} fallback={r.room_type} />,
  },
  {
    name: "check_in",
    label: "Dates",
    render: (r) => (
      <div className="text-xs">
        ...
        {nightsBetween(r.check_in, r.check_out) > 0 && (
          <div className="text-muted-foreground">
            {nightsBetween(r.check_in, r.check_out)} night
  ...
  { name: "status", label: "Status", render: (r) => <StatusBadge value={r.status} /> },
  {
    name: "source",
    label: "Source",
    render: (r) => <span className="text-xs capitalize">{r.source}</span>,
  },
];
