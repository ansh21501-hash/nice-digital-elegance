...
function Cms() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setRows((await adminSettingsList()) as Block[]);
    } catch (e: unknown) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await adminSettingSave({
        data: {
          key: form.key.trim(),
          value: { label: form.label, type: form.type, value: form.value },
        },
      });
      toast.success("Saved");
      setOpen(false);
    ...
    <div>
      ...
      <div className="mb-4 flex justify-end">
        <Button onClick={openNew} className="bg-[#B98A3E] text-white hover:bg-[#a2772f]">
          <Plus className="mr-1.5 h-4 w-4" /> New Block
        </Button>
      </div>
            ...
            {rows.map((b) => (
              <tr key={b.key} className="border-b border-black/5 last:border-0 hover:bg-[#FAFAF8]">
                <td className="px-4 py-3">
                  <p className="font-medium">{b.value?.label || b.key}</p>
                  <p className="text-xs text-muted-foreground">{b.key}</p>
                </td>
                  ...
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-[#C62828]">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingKey ? "Edit" : "New"} content block</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={form.type}
                onValueChange={(v) => setForm({ ...form, type: v as "text" | "image", value: "" })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
