...
function Menu() {
  const { data: cats = [] } = useRows<{ id: string; name: string }>("menu_categories", {
    orderBy: "sort_order",
    ascending: true,
  });
  ...
  const itemColumns: Column[] = [
    ...
    { name: "price", label: "Price", render: (r) => r.price || "—" },
    {
      ...
      label: "Type",
      render: (r) => (
        <span className={r.is_veg ? "text-[#2E7D32]" : "text-[#C62828]"}>
    ...
    },
    {
      ...
      label: "Badges",
      render: (r) =>
        Array.isArray(r.badges) && r.badges.length ? <StatusBadge value={r.badges[0]} /> : "—",
    },
