  ...
  head: () => ({
    meta: [
      { title: "Book Your Stay — Nice Hotel And Restaurant, Mansa" },
      {
        name: "description",
        content:
          "Reserve one or more luxury rooms at Nice Hotel And Restaurant, Mansa. Choose dates, guests and pay securely online.",
      },
  ...
  errorComponent: () => (
    <div className="container-luxe py-32 text-center">
      <p className="text-muted-foreground">
        We couldn't load availability. Please call us at {site.phone}.
      </p>
    </div>
  ),
...
function BookPage() {
  ...
  useEffect(() => {
    ...
    const m = roomParam
      ? roomTypes.find((r) => r.name.toLowerCase() === roomParam.toLowerCase())
      : null;
  ...
  useEffect(() => {
    ...
    getRoomAvailability({ data: { checkIn, checkOut } })
      .then((a) => {
        if (active) setAvailability(a as Avail[]);
      })
      .catch(() => {
          ...
          try {
            ...
            toast.error(
              err instanceof Error
                ? err.message
                : "We could not confirm your booking. Contact us with your payment ID.",
            );
          } finally {
            setPaying(false);
          }
            ...
            {step === 1 && (
              <div>
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <label className="text-xs text-muted-foreground">
                    <span className="mb-1.5 flex items-center gap-1.5">
                      <CalendarDays className="h-4 w-4 text-gold" /> Check-in
                    </span>
                    <input
                      type="date"
                      min={todayStr()}
                  ...
                  <label className="text-xs text-muted-foreground">
                    <span className="mb-1.5 flex items-center gap-1.5">
                      <CalendarDays className="h-4 w-4 text-gold" /> Check-out
                    </span>
                    <input
                      type="date"
                ...
                <label className="mt-4 block text-xs text-muted-foreground">
                  <span className="mb-1.5 flex items-center gap-1.5">
                    <CalendarDays className="h-4 w-4 text-gold" /> Number of nights you want to stay
                  </span>
                ...
                {nights > 0 && (
                  <p className="mt-4 text-sm text-charcoal">
                    {nights} night{nights > 1 ? "s" : ""} selected · {nights + 1} day
                    {nights + 1 > 1 ? "s" : ""}
                  </p>
                )}
              </div>
                  ...
                  {lines.map((l, idx) => {
                    ...
                    return (
                      <div key={l.key} className="rounded-2xl border border-border p-5">
                        ...
                        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            >
                              {roomTypes.map((r) => (
                                <option key={r.id} value={r.id}>
                                  {r.name} — {inr(Number(r.price))}/night
                                </option>
                              ))}
                            </select>
                            {a && (
                          ...
                          <label className="text-xs text-muted-foreground">
                            ...
                            <input
                              ...
                              value={l.adults}
                              onChange={(e) =>
                                updateLine(l.key, { adults: Math.max(1, Number(e.target.value)) })
                              }
                              className={fieldCls}
                            />
                          </label>
                          <label className="text-xs text-muted-foreground">
                            ...
                            <input
                              ...
                              value={l.children}
                              onChange={(e) =>
                                updateLine(l.key, { children: Math.max(0, Number(e.target.value)) })
                              }
                              className={fieldCls}
                            />
                          </label>
                          ...
                          <label className="text-xs text-muted-foreground sm:col-span-2">
                            <span className="mb-1.5 block">
                              Special requests for this room (optional)
                            </span>
                            <input
                              value={l.notes}
                        ...
                        {c && nights > 0 && (
                          <p className="mt-3 text-right text-sm text-charcoal">
                            {inr(c.unitPrice)} × {l.quantity} = <strong>{inr(c.lineTotal)}</strong>
                          </p>
                        )}
                      </div>
                    );
            ...
            {step === 4 && (
              <div>
                    ...
                    <div key={c.line.key} className="rounded-xl border border-border p-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-charcoal">
                          Room {i + 1}: {c.room?.name} × {c.line.quantity}
                        </span>
                        <span className="text-gold">{inr(c.lineTotal)}</span>
                      </div>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {c.line.adults} adult{c.line.adults > 1 ? "s" : ""}
                ...
                <p className="mt-3 flex items-center justify-center gap-1.5 text-[11px] text-muted-foreground">
                  <ShieldCheck className="h-3.5 w-3.5 text-gold" /> Secured by Razorpay
                </p>
              </div>
            )}
              ...
              <>
                ...
                <div className="mt-4 space-y-2 border-t border-border pt-4 text-sm">
                  ...
                  <div className="flex justify-between font-display text-lg text-charcoal">
                    <span>Total</span>
                    <span className="text-gold">{inr(priced.grandTotal)}</span>
                  </div>
                </div>
              </>
            ) : (
