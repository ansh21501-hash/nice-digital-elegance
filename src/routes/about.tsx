  ...
  head: () => ({
    meta: [
      { title: "About — Nice Hotel And Restaurant, Mansa" },
      {
        name: "description",
        content:
          "Discover Nice Hotel & Restaurant in Mansa, Punjab — 9 luxury rooms, fine dining and a grand ballroom for 100 guests. Established 2024.",
      },
        ...
        <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-2">
          <Reveal className="relative overflow-hidden rounded-2xl shadow-luxe">
            <img
              ...
              loading="lazy"
            />
            <span className="absolute bottom-4 left-4 rounded-full bg-charcoal/80 px-4 py-1.5 text-xs tracking-wider text-ivory">
              Nice Hotel — Est. 2024
            </span>
          </Reveal>
          <Reveal delay={0.15}>
            ...
            {aboutStory.map((p) => (
              <p key={p} className="mt-5 text-muted-foreground">
                {p}
              </p>
            ))}
          </Reveal>
        ...
        <div className="mt-12 grid grid-cols-1 gap-10 lg:grid-cols-2">
          ...
          <Reveal delay={0.15}>
            <div className="flex items-center gap-2 text-sm text-gold">
              <Icon name="building" className="h-5 w-5" /> 100 Guests Capacity
            </div>
            <h3 className="mt-2 font-display text-3xl text-charcoal">Grand Ballroom</h3>
            <p className="mt-4 text-muted-foreground">
              Elegant ballroom with crystal chandeliers and sophisticated decor. Perfect for
              parties, kitty parties, and small celebrations. Whether you are hosting an intimate
              kitty party, a joyous birthday celebration, or a small, cherished wedding reception,
              our ballroom provides the perfect, versatile canvas. Our dedicated events team is
              committed to transforming your vision into reality.
            </p>
            <div className="mt-5 flex flex-wrap gap-2">
              {ballroomFeatures.map((f) => (
          ...
          <Reveal>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {diningFeatures.map((f) => (
                <span key={f} className="rounded-full bg-card px-4 py-1.5 text-xs text-brown">
                  {f}
                </span>
              ))}
            </div>
            <div className="mt-6 space-y-2">
              {diningHours.map((h) => (
                ...
                >
                  <span className="text-charcoal">{h.meal}</span>
            ...
            <div className="mt-7">
              <Link to="/menu">
                <LuxeButton>View Menu</LuxeButton>
              </Link>
            </div>
          </Reveal>
          ...
          <Reveal>
            ...
            <ul className="mt-8 space-y-3">
              {locationPoints.map((p) => (
                <li key={p} className="flex items-center gap-3 text-charcoal">
